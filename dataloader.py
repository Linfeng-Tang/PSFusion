...
def random_scale(im, lb, scales):
    scale = random.choice(scales)
    sh = int(im.shape[0] * scale)
    sw = int(lb.shape[1] * scale)
    im = cv2.resize(im, (sw, sh), interpolation=cv2.INTER_LINEAR)
    lb = cv2.resize(lb, (sw, sh), interpolation=cv2.INTER_NEAREST)
    return im, lb, scale

class TrainPre(object):
    def __init__(self, scales=[0.5, 0.75, 1, 1.25, 1.5, 1.75], image_height=480, image_width=640):
        self.scales = scales        
    ...
    def __call__(self, im_lb):
        im = im_lb['im']
        lb = im_lb['lb']
        im, lb = random_mirror(im, lb) #随机翻转
        if self.scales is not None:
            im, lb, scale = random_scale(im, lb, self.scales) ##随机尺度缩放 万一scale比1小 那如何裁剪呢？

        im = normalize(im, self.norm_mean, self.norm_std) ## 归一化至[-1, 1]
        crop_size = (self.image_height, self.image_width)
        
        crop_pos = generate_random_crop_pos(im.shape[:2], crop_size)

        p_im, _ = random_crop_pad_to_shape(im, crop_pos, crop_size, 0)  ## 裁剪或padding
        p_lb, _ = random_crop_pad_to_shape(lb, crop_pos, crop_size, 255)
        p_im = p_im.transpose(2, 0, 1)        
        p_lb = np.array(p_lb).astype(np.int64)[np.newaxis, :]
...
class ValPre(object):
    ...
    def __call__(self, im_lb):
        ...
        lb = im_lb['lb']
        im = normalize(im, self.norm_mean, self.norm_std) ## 归一化至[-1, 1]
        p_im = im.transpose(2, 0, 1)        
        p_lb = np.array(lb).astype(np.int64)[np.newaxis, :]
